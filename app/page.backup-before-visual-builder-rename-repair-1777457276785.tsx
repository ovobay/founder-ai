...
type visual builderDesignCompetitiveStatus =
  | "competitive"
  | "promising"
  | "behind";

type visual builderDesignFinding = {
  id: string;
  label: string;
  status: visual builderDesignCompetitiveStatus;
  detail: string;
  recommendation: string;
};

type visual builderDesignCompetitivenessReport = {
  status: visual builderDesignCompetitiveStatus;
  label: string;
  score: number;
  summary: string;
  findings: visual builderDesignFinding[];
};
...
function visual builderDesignCompetitivenessPanel({
  previewState,
  files,
}: {
  previewState: PreviewState;
  ...
  const report = getvisual builderDesignCompetitivenessReport({
    previewState,
    files,
  });
  ...
  return (
    ...
    >
      ...
      >
        {report.findings.map((finding) => (
          <visual builderDesignFindingCard key={finding.id} finding={finding} />
        ))}
      </div>
    </section>
  );
...
function visual builderDesignFindingCard({
  finding,
}: {
  finding: visual builderDesignFinding;
}) {
  const tone =
...
function getLaunchReadinessReport({
  ...
  const claudeCompetitiveness = getvisual builderDesignCompetitivenessReport({
    previewState,
    files,
  });
  ...
  const signals: LaunchReadinessSignal[] = [
    ...
    },
    {
      ...
      detail: `Visual builder competitiveness is ${claudeCompetitiveness.label} at ${claudeCompetitiveness.score}%.`,
    },
    {
      id: "visual-qa",
      label: "Visual QA",
...
function getVisualQaHealthReport({
  ...
  const visualBuilderReport = getvisual builderDesignCompetitivenessReport({
    previewState,
    files,
  });
  ...
  const behindvisual builderFindings = visualBuilderReport.findings.filter(
    (finding) => finding.status === "behind"
  );

  const promisingvisual builderFindings = visualBuilderReport.findings.filter(
  ...
  const findings: VisualQaHealthFinding[] = [
    ...
    },
    {
      ...
      status: behindvisual builderFindings.length === 0 ? "ready" : "needs-review",
      detail:
        behindvisual builderFindings.length === 0
          ? "No behind-benchmark visual builder findings detected."
          : `${behindvisual builderFindings.length} visual builder benchmark gap${behindvisual builderFindings.length === 1 ? "" : "s"} need improvement.`,
    },
    {
      id: "review-backlog",
      label: "Review backlog",
      status:
        designReviewFindings.length + promisingvisual builderFindings.length === 0
          ? "ready"
          : "needs-review",
      detail:
        designReviewFindings.length + promisingvisual builderFindings.length === 0
          ? "No design review backlog detected."
          : `${designReviewFindings.length + promisingvisual builderFindings.length} design item${designReviewFindings.length + promisingvisual builderFindings.length === 1 ? "" : "s"} need review.`,
    },
  ];
...
function createVisualQaChecklistMarkdown({
  ...
  const lines = [
    ...
    "",
    ...(behindvisual builderFindings.length > 0
      ? behindvisual builderFindings.map(
          (finding) =>
    ...
    "",
    ...(promisingvisual builderFindings.length > 0
      ? promisingvisual builderFindings.map(
          (finding) =>
            `- [ ] ${finding.label}: ${finding.detail} Recommendation: ${finding.recommendation}`
        )
...
function createvisual builderDesignCompetitivenessMarkdown({
  previewState,
  files,
}: {
  previewState: PreviewState;
...
function createvisual builderDesignCompetitivenessSummary({
  previewState,
  files,
}: {
  previewState: PreviewState;
...
function getvisual builderDesignCompetitivenessReport({
  previewState,
  files,
}: {
  previewState: PreviewState;
  ...
  const findings: visual builderDesignFinding[] = [
    {
      id: "visual-ambition",
...
function DeployReadinessPanel({
  ...
  async function handleExportvisual builderCompetitivenessReview() {
    setIsExportingvisual builderCompetitiveness(true);
    setvisual builderCompetitivenessMessage("");
    setWorkspaceError("");

    const filePath = "config/visual-builder-competitiveness.md";
    const contents = createvisual builderDesignCompetitivenessMarkdown({
      previewState,
      files,
    });
    ...
    try {
      ...
      setvisual builderCompetitivenessMessage(message);
      setWorkspaceError(message);
    } finally {
      setIsExportingvisual builderCompetitiveness(false);
    }
  }



  async function handleCopyvisual builderCompetitivenessSummary() {
    setCopyvisual builderCompetitivenessMessage("");
    setWorkspaceError("");

    const summary = createvisual builderDesignCompetitivenessSummary({
      previewState,
      files,
    });
    ...
    try {
      await navigator.clipboard.writeText(summary);
      setCopyvisual builderCompetitivenessMessage("Visual builder competitiveness summary copied.");
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
      ...
      setCopyvisual builderCompetitivenessMessage(message);
      setWorkspaceError(message);
    }
  }
