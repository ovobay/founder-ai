...
/**
 * Adds database-level project isolation hardening.
 *
 * Your API routes already mostly scope records by owner_user_id.
 * This migration makes the database enforce the same rule:
 *
 * - founder_builds.owner_user_id must match founder_projects.owner_user_id
 * - founder_project_files.owner_user_id must match founder_projects.owner_user_id
 * - founder_project_files.last_build_id, when present, must belong to the same
 *   owner and project
 *
 * Why:
 * The API uses getSupabaseAdmin(), which bypasses RLS.
 * Manual checks are good, but DB-level guards stop future route mistakes from
 * becoming a data leak with a UI.
 */
...
const migrationPath = path.join(
  root,
  "supabase/migrations/0004_founder_ai_isolation_hardening.sql"
);

const migrationSql = `create or replace function public.enforce_founder_build_project_owner()
returns trigger
language plpgsql
as $$
declare
...
if (fs.existsSync(migrationPath)) {
  const backupPath = `${migrationPath}.backup-${Date.now()}`;
...
console.log("");
console.log("✅ Added project isolation hardening migration.");
console.log(`Wrote: ${migrationPath}`);
console.log("");
console.log("Next:");
console.log("1. Apply the migration to Supabase.");
console.log("2. Restart the dev server.");
console.log("3. Test with two users.");